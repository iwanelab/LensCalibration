...
点対応だけで先に高精度化してから、最後に路面線分マッチングで仕上げるという順序を採用している(路面線分マッチングは高精度なposeを前提にした検証的な性質が強く、粗いposeの段階でいきなり路面線分に頼ると、後段の判定基準(角度・距離の許容範囲)がposeの誤差と線分自体の対応誤差を区別できなくなるため)。
...
## 各ステップでのカメラパラメータ改善を目視確認する

各ステップでカメラパラメータがどれだけ改善されているかは、赤・シアンオーバーレイ(R=実写、G=B=点群レンダリング。ズレは色収差のように見え、アライメントが一致するとグレーに揃う)で追うことができます。frame 00374を例に、ステップを追って見ていきます。

| ステージ | fx | fy | cx | cy | k1 | k2 |
|---|---|---|---|---|---|---|
| ステップ2(概略) | 904.41 | 885.06 | 967.04 | 493.35 | -0.03774 | -0.02765 |
| ステップ6(高精度、非路面) | 908.09 | 903.68 | 984.71 | 527.54 | -0.06626 | -0.00529 |
| ステップ10(最終、点+線分) | 908.58 | 905.19 | 984.32 | 529.78 | -0.06879 | -0.00419 |

***表** — ステージを追うごとのカメラパラメータの変化。特にfy・cyがステップ2→6で大きく動いた後、ステップ6→10ではわずかな調整に収束している*

### ステップ1: 参照画像とのALIKED/LightGlueマッチング

まず、指定した参照画像とのALIKED/LightGlueマッチングで対応点を得ます。

![Frame 00374 matched against reference image cam00_frm00230 via ALIKED/LightGlue](readme_images/fig_step1_ref_match.jpg)

***図2** — frame 00374と参照画像cam00_frm00230のALIKED/LightGlueマッチング結果(緑の線が対応点、n=604)*

この対応点だけから求めた概略カメラパラメータ(ステップ2)で点群をレンダリングし、実写と重ね合わせると、まだ大きくズレています。横断歩道の縞が二重に見え、建物の輪郭にも色収差のようなズレが出ています。

![Overlay using the approximate pose derived only from Step 1's point correspondences](readme_images/fig_overlay_step1.jpg)

***図3** — ステップ1の点対応だけから求めた概略パラメータでのオーバーレイ(frame 00374)*

### ステップ3〜8: 点群レンダリングとの反復マッチング

概略パラメータで点群をレンダリングし、そのレンダー画像と実写を改めてALIKED/LightGlueでマッチングし直す、を収束するまで繰り返します(既定最大5回)。反復が進みposeの精度が上がるほどレンダーが実写に近づくため、対応点も増えていきます。

![Render-vs-photo matching at iteration 1](readme_images/fig_rendermatch_iter1.jpg)

***図4** — 反復1回目のレンダー(左)↔実写(右)マッチング(n=1253)*

![Render-vs-photo matching at iteration 5](readme_images/fig_rendermatch_iter5.jpg)

***図5** — 反復5回目(収束後)のレンダー(左)↔実写(右)マッチング(n=2591、図4の約2倍)*

この反復後の高精度パラメータ(非路面点のみ)でのオーバーレイは以下です。ステップ1直後(図3)と比べ、横断歩道の縞のズレがほぼ解消し、建物の輪郭も揃っています。

![Overlay using the high-precision (non-road) pose after the iterative refinement loop](readme_images/fig_overlay_step6.jpg)

***図6** — ステップ3〜8の反復後(非路面点のみ)のオーバーレイ(frame 00374)*

### ステップ9: 路面線分マッチング

収束した高精度パラメータを前提に、路面の線分をLSD/HoughLinesPで検出し(赤)、参照画像側の線分と3D的にマッチングが成立した点(緑)を重ねます。

![Frame 00374 line detection and 3D line matching result](readme_images/fig_line_match.jpg)

***図7** — frame 00374の路面線分マッチング結果。赤=検出された全線分候補(527本)、緑の点=3D線分マッチング成立点(70件)*

### ステップ10: 最終フィット

点対応+線分対応の両方を使った最終フィットの結果が、冒頭の図1(実写↔レンダーの交互表示)です。
