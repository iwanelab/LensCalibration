...
![Best result: frame 00374 flicker comparison after ref_topk=8](readme_images/fig01.gif)
...
MMSは**Ladybug**(全方位を一度に撮影する多眼カメラ)とLiDAR(レーザー光で周囲の形状を計測するセンサー)を搭載した計測車両で、これによって位置が既知の参照データを事前に取得しておきます。このデータを**CVImageCreator**というツールに与えると、キューブマップ化された全周映像と各フレームのカメラ位置・姿勢(CV値)を含む**izic**ファイル、およびLiDARによる3次元点群の**laz**ファイルが得られます。この izic と laz を**COLMAP**形式に変換したものが、Stage1で使われた参照データです。**COLMAP**は、複数の画像から3次元形状やカメラ位置を復元するStructure-from-Motion(SfM)ツールとして広く使われているオープンソースソフトウェアで、カメラパラメータ・各画像の位置姿勢・3次元点群をテキストファイル(cameras.txt / images.txt / points3D.txtなど)で表現する標準的なフォーマットとしても使われています。本パイプラインではこのCOLMAP形式を**データの受け渡し形式**として使っているだけで、COLMAP自体によるマッチング・SfM計算は行っていません(位置・姿勢・点群はすべてMMSの直接計測から得られた値です)。
...
![frame 00374 flicker comparison](readme_images/fig02.gif)

***図4** — 実際のflicker比較（写真と点群レンダリングを交互表示、frame 00374）。奥の建物群はほぼ一致して見える一方、手前の横断歩道にはっきりとしたズレが見える*
...
***図5** — 実際のStage1マッチング結果(標準設定のALIKED/LightGlue、frame 00374と最も近い参照画像)。緑・水色などの対応点はすべて奥の建物に集中しており、手前の横断歩道(両画像とも高コントラストで写っている)には1点も対応点が出ていない*
...
Stage5までの結果でまだ対応点が無い領域を、古典的な線分検出(LSD)を使って補うステージです。線分同士の交点は、線分そのもの(アパーチャ問題の影響を受けやすい)と違って位置が一意に定まるため、参照画像側の交点をStage5までの姿勢で再投影し、ダッシュカム画像側で実際に検出された交点がその近くにあれば対応点として採用します。建物の角のような交点では高精度に機能しますが、横断歩道のような路面標示が作る交点は依然としてマッチングの手がかりとしては弱く、路面付近の精度改善という点ではStage8ほど信頼できません。
...
下図は、特徴点マッチングの結果ではなく、**点群そのもの**を可視化したものです。参照画像を撮影したカメラの位置・向き(既知)を使って、LiDAR点群の各点を3次元空間から参照画像の2次元ピクセル上に再投影し、赤い点として重ねて描画しています。

![Full point cloud vs. road-and-marking-only segmented point cloud, reprojected onto the same reference image](readme_images/fig04.jpg)

***図6** — 特徴点ではなく**点群(LiDARの3次元点)**を、同じ参照画像に再投影して赤い点で表示(実データ、cam00_frm00235)。A: フルの点群を再投影 — 車・歩行者・歩道にも点が無差別に現れる。B: 車道・道路標示だけを抽出した点群を再投影 — 車道・横断歩道の領域だけに点が絞られている*
...
***図7** — ALIKED / LightGlueマッチングが2回登場する理由と、それぞれの違い*
...
**[Step1: 独立マッチング]** — 7,015件 — 見た目(ALIKED/LightGlue)だけによる対応点候補(8枚の参照画像分を合計)

↓ 路面点群と重なるものだけを残す(Step2) — 5,916件(84%)を建物・車・歩道など路面以外として除外

**[路面点群と重なる候補]** — 1,099件 — ここまでは「見た目」だけに基づく判定

↓ 位置情報からの逆算と一致するか検証(Step3・Step4) — 297件(27%)を不一致として棄却

**[クロス検証を通過した対応点]** — 802件 — 見た目と位置情報の両方が一致した、信頼できる対応点

***図9** — frame 00374(参照画像8枚をプール)における実際の対応点数の推移(パイプラインのログ値)。路面点群による絞り込み(Step2)だけでなく、位置情報との一致確認(Step4)によっても、見た目だけでは検出できない誤マッチングの一部(1,099件中297件、27%)が除外されている*
...
***図10** — frame 00374における路面点群クロス検証マッチングのStep1採用点(road_crossval_vis)。A: 参照画像1枚のみ(ref_topk=1) — 横断歩道の縞模様の上にはほとんど点が乗っていない。B: ray-alignment上位8枚をプール(ref_topk=8) — 縞模様のほぼ全体に点が広がっている*
...
***図11** — 参照画像を複数(ref_topk)使うことによる路面領域対応点の改善(全10フレーム集計)*
...
> **結果**
>
> これが現時点で最良の結果であり、`--road_match_ref_topk 8`を含む本節の変更はすべて既定値になっています。冒頭の図1のflicker比較は、この設定で得られた結果です。
...
**[対応点]** — 観測位置（2Dピクセル）+ 3D座標（点群中の1点）

↓ 現在推定されているカメラの位置・姿勢・レンズ歪み（魚眼モデル）で再投影

**[再投影位置]** — 3D座標を現在のposeで逆算した2Dピクセル位置

↓

**誤差 e（px）＝ 観測位置と再投影位置のユークリッド距離**

**[路面領域だけに絞る]** — 3D座標が車道・道路標示点群の最近傍点から0.3以内のものだけを残す

→

**[集計]** — 誤差の中央値 / 誤差15px未満の割合

***図12** — 誤差の求め方。実際の最適化計算(joint bundle adjustment)が使うデータ選択を再現した上で、路面領域だけに絞って集計している*
...
***図13** — 図12の手順を実際のフレームの対応点に適用した例(いずれも路面点群クロス検証マッチングで採用された点)。**1.** frame 00374、誤差 e = 1.91px — 大半の対応点はこの程度小さく、拡大しないと赤(観測位置)と青(再投影位置)の違いは見えない。**2.** frame 00333、誤差 e = 41.0px — 許容範囲内(このフレームの奥行きでの許容ピクセル数以内)ではあるが、ズレが目視でもはっきり分かる例*
...
最終的に求まったカメラの位置・向き・レンズの歪みを使い、点群をダッシュカム視点に再投影して検証用の画像・GIFを生成するステージです。本資料で使っている実写真と点群レンダリングのflicker比較(図1・図4など)も、このStage9の出力です。
