...
**ALIKED/LightGlueと線分マッチングの相補性**: 防犯カメラ写真(`00001.jpg`)から実際に抽出された特徴点・線分の**全件**(マッチの成否を問わない)と、**実際にマッチングに成功した分だけ**を見比べると、この相補性がはっきり分かります。
...
***図16** — 上記のうち、実際に参照画像とのマッチングに成功した分だけ(シアン=ALIKED/LightGlue対応点n=1097、赤=paint線分マッチング・マゼンタ=other線分マッチング計1798件)。ALIKED/LightGlueの対応点は建物・樹木といった模様の多い場所にほぼ限定され、横断歩道など路面の平行線ではアパーチャ問題によりほとんど採用されない一方、線分マッチングはまさにその路面(特に横断歩道)を集中的に補っている*
...
同様に、線分の種別分類(`paint`/`other`)についても、明るさの閾値が緩すぎて、明るめの舗装補修痕(白塗装ほど明るくないが閾値は超える)が`paint`に誤分類されるケースが見つかりました。当初は実測分布(真の白塗装はbright中央値250・contrast中央値92、その他は95%点でもbright=242・contrast=74.4)を見て閾値を手動で引き上げましたが、撮影条件(露出・時間帯)が変わるたびに再チューニングが必要になるため、最終的には**画像ごとに検出された線分群の明るさ・コントラスト分布からOtsu法で閾値を自動決定する**(`_auto_paint_thresholds`)方式に変更しました。`00001.jpg`ではOtsu法はbright≈138・contrast≈69を自動算出し、手動閾値による分類との一致率92.3%、既知の誤分類ケース(ID176)も引き続き`other`に正しく分類されることを確認済みです。
