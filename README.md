...
参照データ自身の精度を確認しておきます。下図は、参照画像(cam00_frm00233)に、同じ参照データの点群を、その画像自身の(MMSの直接計測による、推定を一切介さない)既知の位置・姿勢だけで再投影したflicker比較です。ダッシュカムの位置推定は一切関与していません。
...
***図3** — 参照画像(cam00_frm00233)に、参照データ自身の位置・姿勢(MMSの直接計測、推定なし)だけを使って点群を再投影したflicker比較。建物の輪郭・窓の格子・横断歩道の縞模様まで、目立ったズレなく重なっている*
...
***図5** — 実際のflicker比較（写真と点群レンダリングを交互表示、frame 00374）。奥の建物群はほぼ一致して見える一方、手前の横断歩道にはっきりとしたズレが見える*
...
***図7** — 特徴点ではなく**点群(LiDARの3次元点)**を、同じ参照画像に再投影して赤い点で表示(実データ、cam00_frm00235)。A: フルの点群を再投影 — 車・歩行者・歩道にも点が無差別に現れる。B: 車道・道路標示だけを抽出した点群を再投影 — 車道・横断歩道の領域だけに点が絞られている*
...
最終的に求まったカメラの位置・向き・レンズの歪みを使い、点群をダッシュカム視点に再投影して検証用の画像・GIFを生成するステージです。本資料で使っている実写真と点群レンダリングのflicker比較(図1・図5など)も、このStage9の出力です。
