...
この結果で求まったレンズモデルとパラメータは以下の通りです(Stage4終了時点、詳細は本文)。

| 項目 | 値 | 説明 |
|---|---|---|
| レンズモデル | 等距離射影(equidistant)ベースの魚眼モデル | θ_d = θ(1 + k1θ² + k2θ⁴)(θ: 入射角、θ_d: 歪みを含めた投影角。ダッシュカム側と同じくk1・k2のみを使用) |
| fx | 1092.51 | 焦点距離(x方向、px) |
| fy | 1088.58 | 焦点距離(y方向、px) |
| cx | 1018.97 | 主点(x座標、px) |
| cy | 536.04 | 主点(y座標、px) |
| k1 | 0.0200 | 歪み係数1 |
| k2 | -0.0486 | 歪み係数2 |
...
カメラの位置(`camera_pos`)は現地測量により既知なので、求めるべき自由度は「回転R(3) + 内部パラメータfx,fy,cx,cy,k1,k2(6)」の**計9自由度**のみです。
...
`01_matching/vis/`の個別マッチング結果を目視確認したところ、**1ペアあたりのgeo-verified対応点数が20未満のペアは、幾何検証(RANSAC)を通過していてもほぼ全て誤マッチ**であることが判明しました。下図はその一例で、n=3の「合格」対応点がありますが、線で結ばれた2点は明らかに違う場所を指しています(点数が少ないRANSACは、たまたま整合する誤対応を通しやすい)。
...
***図3** — `01_matching/vis`の実例(`cam00_frm00000` vs `00001.jpg`)。「n=3 geo-verified matches」と表示されているが、緑・赤・青の対応線はいずれも明後日の方向を指しており、幾何検証を通過していても内容的には誤マッチであることが分かる*

実際に集計すると、全590ペア中472ペア(80%)が20件未満で、そこに含まれる点数は全体の約3割を占めていました。**誤対応がプールに混入すると、それを無理やり矛盾なく説明しようとしてk1,k2が暴走する**、というのが根本原因でした(モデルの不安定性ではなく、データそのものの質の問題)。
...
|---|---|---|
| k1, k2 | 0.137, **-0.9255** | 0.083, **-0.151**(物理的に妥当) |
| フィット集合のRMS | 563px | **118px** |
| 15px以内の割合 | 60.6% | **77.3%** |
| レンダリング | 破綻(図2) | 正常(図4) |
...
|---|---|---|
| k1, k2 | 0.083, -0.151 | **0.020, -0.049**(より典型的な魚眼レンズ値) |
| フィット集合のRMS | 118px | **7.2px** |
| 15px以内の割合 | 77.3% | **96.9%** |
...
**Stage5前後での横断歩道の見え方の変化**: 防犯カメラ写真(赤チャンネル)と点群レンダリング(シアンチャンネル)を重ね合わせ、ズレを色の縁取りとして可視化しました(重なっていれば白、ズレていれば赤/シアンの縁取りが出ます)。

![Crosswalk stripe alignment before Stage5 (Stage4 only)](readme_images/fig12.png)

***図12** — Stage5適用前(Stage4まで)。横断歩道の縞の輪郭に沿って、上側がシアン・下側が赤という一貫したズレが縞ごとに見える*

![Crosswalk stripe alignment after Stage5](readme_images/fig13.png)

***図13** — Stage5適用後(最終結果)。同じ場所の縞の輪郭のズレがほぼ解消され、白く一致して見える範囲が大幅に広がっている*
...
実例: ある参照線分(長さ0.39m)と実写側の線分(長さ0.66m)は、最短点間距離ではわずか0.022mでしたが、**重心間距離では0.257m**も離れていました。短い方の線分の端が、無関係な長い線分の端に「手を伸ばして」たまたま触れていただけで、線分の主要部分同士は全く重なっていませんでした。
